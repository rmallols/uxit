describe('roleService', function() {
    ...
    describe('getRole', function() {
        ...
        it('should return the reader role', function() {
            var role = rS.getRole(1);
            expect(role.karma).toBe(1);
            expect(role.code).toBe(cS.roles.reader);
        });

        it('should return the creator role', function() {
            var role = rS.getRole(2);
            expect(role.karma).toBe(2);
            expect(role.code).toBe(cS.roles.creator);
        });

        it('should return the admin role', function() {
            var role = rS.getRole(3);
            expect(role.karma).toBe(3);
            expect(role.code).toBe(cS.roles.admin);
        });

        it('should return the super admin role', function() {
            var role = rS.getRole(4);
            expect(role.karma).toBe(4);
            expect(role.code).toBe(cS.roles.superAdmin);
        });
    });
    ...
    describe('hasGuestRole', function() {
        ...
        it('should determine that super admin users have the guest role', function() {
            var user = { role: 4 };
            expect(rS.hasGuestRole(user)).toBe(true);
        });
    });
    ...
    describe('hasReaderRole', function() {
        ...
        it('should determine that super admin users have the reader role', function() {
            var user = { role: 4 };
            expect(rS.hasReaderRole(user)).toBe(true);
        });
    });
    ...
    describe('hasCreatorRole', function() {
        ...
        it('should determine that super admin users have the creator role', function() {
            var user = { role: 4 };
            expect(rS.hasCreatorRole(user)).toBe(true);
        });
    });
    ...
    describe('hasAdminRole', function() {
        ...
        it('should determine that super admin users have the admin role', function() {
            var user = { role: 4 };
            expect(rS.hasAdminRole(user)).toBe(true);
        });
    });

    describe('hasSuperAdminRole', function() {

        it('should determine that non logged users doesn\'t have the super admin role', function() {
            var user = null;
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that users without role have the super admin role', function() {
            var user = { role: null };
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that guest users doesn\'t have the super admin role', function() {
            var user = { role: 0 };
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that reader users have the super admin role', function() {
            var user = { role: 1 };
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that creator users doesn\'t have the super admin role', function() {
            var user = { role: 2 };
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that admin users doesn\'t have the super admin role', function() {
            var user = { role: 3 };
            expect(rS.hasSuperAdminRole(user)).toBe(false);
        });

        it('should determine that super admin users have the super admin role', function() {
            var user = { role: 4 };
            expect(rS.hasSuperAdminRole(user)).toBe(true);
        });
    });
